...
export default function PackEruption({
  ...
  const eruptionRef = useRef<Animated.CompositeAnimation | null>(null);
  const eruptionStartedRef = useRef(false);
  ...
  if (!visible) {
    ...
    eruptionRef.current?.stop();
    eruptionStartedRef.current = false;
    return;
  }
  ...
  introAnimation.start(({ finished }) => {
    if (!finished || eruptionStartedRef.current) return;

    shakeLoopRef.current = Animated.loop(shakeSequence);
...
useEffect(() => {
  if (!visible || cards.length === 0 || eruptionStartedRef.current) {
    return;
  }

  eruptionStartedRef.current = true;
  shakeLoopRef.current?.stop();
  ...
  eruptionRef.current.start(({ finished }) => {
    if (finished) {
      completeRef.current?.();
    }
  });
    ...
    >
      ...
      >
        ...
        {cards.slice(0, 3).map((pull, index) => {
          ...
          return (
            <Animated.Image
              key={`eruption-${index}-${pull.card.id}`}            source={{ uri: pull.card.front_image_url }}
              resizeMode="contain"
              style={{
                position: 'absolute',
                top: TOP_H - 5,
